...
def fetch_podcasts_from_database(database_path):
    ...
    for row in cursor.execute(
        "SELECT title, img, mp3_file, shortdesc, mtime FROM episodes"
    ):
        title, img, mp3_file, shortdesc, mtime = row
        podcast = {
            "title": title,
    ...
    for podcast_data in podcasts_data:
        ...
        for s3_object in s3_objects:
            ...
            if podcast_data["mp3_file"] == s3_object:
                podcast_data["file_url"] = (
                    f"https://podcast-fl.s3.eu-north-1.amazonaws.com/MP3_PODCAST/{s3_object}"
                )
